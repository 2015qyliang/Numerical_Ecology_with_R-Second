...
# **第六章：典型排序 -- Canonical Ordination | 约束性排序**
...
对称分析, 意味着两个或者多个矩阵在分析过程中扮演相同的角色, 它们之间既不依赖也不相互解释. 
对称和非对称排序方法的选择取决于是要进行相关性分析(对称)还是要进行回归分析(非对称). 
相关性分析需要更多的描述性或者解释性信息, 当模型中没有嵌入单向因果假设时亦可适用; 然而, 回归性分析具有更多的推断性判断, 通过解释性变量之间的线性组合来解释响应变量的变化. 
两种分析方法源于不同的分析目的, 不应该相互对立而存在. 

在生态学研究中, 存在三种对称性分析方法: 1-> 典型相关分析 canonical correlation analysis (CCorA), 2-> 共惯性分析 co-inertia analysis (CoIA), 3-> 多因子分析 multiple factor analysis (MFA).
...
CCorA 由两个数据表计算而来, 该方法的目的是表示沿典型轴的观测值, 该观测值可以最大化两个表之间的相关性; 该方法通过最大限度地利用两组变量之间的协方差矩阵()来表示两组变量之间的 组内离散度. 
两组变量必须是定量的, 而且是呈多元正态分布的; 两个数据矩阵中的变量总量必须小于 n-1. 
在 CCorA 分析中, 还可以检验两个多元数据集的线性独立性假设. 

RDA 和 CCA 两种分析方法的使用使得 CCorA 在生态学研究使用较少, 原因在于多数的生态学问题会优先采用不对称排序的方式从处理(控制)-反应假设的角度来进行描述的. 

CCorA 更适合进行探索性分析或者探索两组变量可能相互影响的案例(这种问题多见于真实的生态问题中), 比如两组竞争性类群的研究, 还有群落定植过程中土壤-植被之间相互关系的长期研究. 
(在每个数据表中的物种数量都小于 n-1 的情况下进行 CCorA 才有可能). 

### 6.8.2 使用 CCorA() 进行典型相关分析(Canonical Correlation Analysis) 

研究两个完整的解释性变量子集之间的相关关系. CCorA 分析中会自动对数据进行标准化. 使用 vegan 工具包中的 CCorA() 函数. 
...
CoIA 是 CCorA() 的一种替代方法(同属于对称性分析方法, 只做变量之间的相关性, 不做变量之间的相互解释), 对于两个或者多个数据集之间的分析更加灵活.  

CoIA 分析的使用: 1-> 计算两个数据集中变量交叉的协方差矩阵, 协方差的平方和 视为 总 co-inertia. 计算特征值和特征向量的矩阵. 特征值代表 总co-inertia 的某一部分. 
2-> 将两个原始数据表的对象和变量投影到共惯量轴上; 通过图的形式上, 比较两个数据表在共惯量空间中的投影. 

CoIA 需要输入 位点-变量 类型的矩阵数据. 

### 6.9.2 使用 ade4 分析包的 coinertia() 进行共惯性分析(Co-inertia Analysis)

由于原处的技术设计原因, 在使用 ade4 工具包进行 coinertia() 分析之前, 首先使用 dudi.pca() 函数进行 PCA 预分析, 或者使用 dudi.pco() 函数进行 PCoA 预分析.

> 两个不同的矩阵变量之间的连线, 表示相关关系, 连线越短则相关性越高 (对称性分析, 不作变量之间的解释, 仅作变量之间的关联)
...
>  RV coefficient 是总惯量除以两个数据表格单独排序的两个总惯量的积的平方根, 可视为 X 和 Y 的 Pearson 相关系数. 
...
MFA 是另一种**对称分析**方法(基于两个以上的数据集), 该方法侧重的还是相关性分析, 不涉及对数据集中的任何因果推断的假设分析. 
分析的数据变量必须属于相同的数据类型(要么是定量, 要么是定性). 
如果所有的变量都是定量的, 那么 MFA 分析基本上就是应用于每一个自己加权的整组变量的主成分分析(PCA). 
**(注, 该处不要与 多元对应分析 multiple correspondence analysis, MCA 相混淆, MCA 分析中针对 单一矩阵中的定量变量进行分析)**. 

MFA 分析步骤: -1> 对定量变量数据(中心化, 最适为标准化)进行 PCA 分析, -2> 重组已加权的数据, 将结果数据集进行 全局PCA 分析, -3> 将变量的不同子集投影到全局结果中, 评估对象和变量的共有结构和差异. 

计算: ade4 工具包中的 mfa() 函数, 或者 FactoMineR 工具包中的 MFA() *(该方法的优点更多)*. 

MFA的分析方法可以扩展到层次数据结构中, 进而形成等级多因素分析(Hierarchical Multiple Factor Analysis, HMFA), 可用于探索高山森林生态系统中植被+土壤+动物+腐殖质之间的结构关系. 
HMFA 的原理是变量最低级层次开始计算每一个级别的MFA, 计算结果被更高一级的层次计算所使用, 其中根据组的数量进行新的加权. 可使用 FactoMineR 工具包中的 HMFA() 进行相应的计算. 

### 6.10.2 使用 FactoMineR 分析包进行多因素分析(MFA)
...
生态位理论会预测已适应 环境的定植的 物种的生态和行为特征. 
功能生态学主要研究物种特征和环境之间的相互关系. 
这种研究方法, 由于要检验假设, 因此面临方法学上的挑战, 必须通过适合的方法评估物种特征与环境特征之间的关系, 如物种的有无或丰度所导致的群落结构特征与环境之间的相互关系. 

下文将介绍两种解决上述问题的方法: -1> RLQ, -2> fourth-corner methods. 
这两种方法具有互补的特性: RLQ 是一种排序方法, 允许三种数据表组合的数据结构的可视化, 但只是一种简单的全局检验; fourth-corner methods 是一系列个体特征与环境相互关系的统计检验, 不考虑个体特征与环境变量之间的协方差, 同时没有关于位点和物种的输出结果. 

本书中介绍了一些提高两种方法互补性的调整: 整体数据由三种矩阵组成, -1> 矩阵A(或者矩阵L) -- 物种有无或者丰度数据 (n 个位点, p 个物种) , -2> 矩阵B(或者矩阵Q) -- 生物学或者行为特征数据 (p 个物种, s 个生物学特征或者行为特征), -3> 矩阵C(或者矩阵R) -- 栖息地特征数据 (n 个位点, m 个栖息地特征).

> 主要目的是为了研究物种特征和环境之间的相互关系. 

RLQ 方法是分析由矩阵L连接起来的矩阵R和矩阵Q (矩阵L在此处起到了链接作用), s*m 矩阵的交叉特征和环境变量 是第四个矩阵(也成为 矩阵D, 第四角)

### 6.11.1 四角法分析(The Fourth-Corner Method)

该方法的原理在于: -1> 估计矩阵D的参数, -2> 检验参数的显著性(差异性), 在六种置换模型中选择最适合的一种.

Model 1: environmental control over individual species. 
Model 2: environmental control over species assemblages. 
Model 3: lottery. 
Model 4: random species attributes. 
Model 5: permute rows and columns. 
Model 6: this is actually a combination of models 2 and 4. 

### 6.11.2 物种功能属性与环境因子关系分析( RLQ Analysis)

RLQ 分析方法是共惯量(co-inertia analysis, CoIA)分析的一个扩展. 

### 6.11.3 在 R 中的应用

该部分数据的科研背景: 阿尔卑斯山上的植物性状对积雪融化梯度的响应. 
主要的科学问题是: 根据植物物种的功能特征评估, 积雪的持续时间及其所有后果如何影响高山草原? 
...
排序, 查看生态数据信息的一种方式, 同时也是在涉及多个数据矩阵的多环境变量中揭示,解释,检验和模拟生态关系的有力的方法家族. 

>本书汇总了作者认为最重要且最有用的方法. 

Name, acronym | Use (examples) | R functions(packages) | Data; implementation; limitations
- | - | - | - 
A. Asymmetric analyses -- 非对称分析 |  |  |  
 Redundancy analysis, RDA | Predict Y with X (Variation partitioning) | rda{vegan} / varpart{vegan} | All types; species data with prior transformation; m < (n-1). Linear model.  
 Canonical correspondence analysis, CCA | Predict Y with X | cca {vegan} | Y: species abundances; X: all types; m < (n-1); unimodal response to latent variables.  
 Linear discriminant analysis, LDA | Explain classification with quantitative variables | lda{MASS} | Y: classification; X: quantitative variables. Linear model.  
 Principal response curves, PRC | Model community response through time in controlled experiments | prc{vegan} | Y: community data; factor “treatment”; factor “time”.  
 Co-correspondence analysis (asymmetricform), CoCA | Predict one community on the basis of another | coca{cocorresp} | Y: data for community 1; X: data for community 2; both at the same sites. Unimodal response to latent variables
B. Symmetric analyses -- 对称分析 |  |  |  
 Co-correspondence analysis (symmetric form), CoCA | Optimized comparison of two communities (descriptive approach) | coca{cocorresp} | Y1: data for community 1; Y2: data for community 2; both at the same sites. Unimodal response to latent variables  
 ...
 Multiple factor analysis, MFA | Common structures of two or more data matrices. | mfa{ade4} / MFA{FactoMineR} | Simultaneous ordination of 2 or more weighted tables. Mathematical type must be homogeneous within each table.  
 RLQ analysis, RLQ | Species traits related to environmental variables | rlq{ade4} | 3 tables: species-by-sites, sites-by-environment; species-by-traits  
 Fourth-corner analysis | Species traits related to environmental variables | fourthcorner, fourthcorner2{ade4} | 3 tables: species-by-sites, sites-by-environment; species-by-traits  
