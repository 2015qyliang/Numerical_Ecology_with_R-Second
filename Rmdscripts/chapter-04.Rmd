...
## 4.5 Ward’s Minimum Variance Clustering - Ward最小方差聚类


```{r}
spe.ch.ward <- hclust(spe.ch, method = "ward.D2") # Ward最小方差聚类
plot(spe.ch.ward,  # 使用默认选项绘制树形图
  labels = rownames(spe), 
  main = "Chord - Ward")
```
...
### 4.7.2 Cophenetic Correlation - 同表象相关
...
为了说明不相似矩阵和从各种方法获得的一组共生矩阵之间的关系，可以绘制 Shepard-like 图
