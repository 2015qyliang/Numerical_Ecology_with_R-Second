...
PCA 以及 CA 强加了对象之间保留的距离: PCA 的 Euclidean 距离(以及物种数据预转换后的其他几个距离)和 CA的χ2距离. 
如果希望根据一些其他相异性测度对对象进行排序 -- PCoA 首选方法. 
PCoA 提供了一系列对象的 Euclidean距离, 对象之间的关系由相异性测度由进行评估. 

与 PCA 和 CA 一样, PCoA 产生一组正交轴, 其重要性由特征值测量. 
由于 PCoA 基于关联矩阵, 因此它可以直接表示 **对象之间的关系(如果关联矩阵处于 Q模式)** 或 **变量之间的关系(如果关联矩阵处于 R模式)**. 
如果预测变量, 变量可以使用相关性或加权平均值与排序轴相关联, 并在排序图上绘制. 
在具有 Euclidean属性(第3.3节)的关联度量的情况下, PCoA 以 Euclidean 的方式表现. 
但是如果使用的关联系数是 非Euclidean, 则 PCoA 可以通过产生 **除正数之外的几个负特征值以及零特征值** 来作出两者之间的反应. 
对应于 **负特征值的轴** 的复杂性, 不能在实际排序轴上表示. 
在大多数应用中, 这不会影响对象在几个第一主轴上的表示, **但如果最大负特征值的绝对值与第一个正特征值相同, 则可能导致问题.** 

几种技术方法解决 负特征根问题: 1> 为对象之间的平方差异添加常数(Lingoes修正); 2> 为相异性度量添加常数; 3> 通过保持特征向量与其原始 Euclidean范数(向量长度= 1)而不是将每个特征向量除以其特征值的平方根, 可以避免复杂的轴(第七章-空间分析中有应用; 由于特征向量没有转化处理, 因此不能使用 PCoA 进行分析, 无法展示对象之间的原始距离). 

分析包 adespatial 中的 dist.ldc() 函数可以检验 相异性系数是否为 欧式, 或者判断 平方根是否为 欧式. 

PCOA 结果图的解读 -- 相似相近原则: 对象在图上的距离的远近表示关联测度上的相似性. 

分析包 vegan 中的 wcmdscale() 函数可以计算 加权的 PCOA, 该方法基于 cmdscale() 函数. 
...
abline(v = 0, 
       lty = 3)
# 添加物种的加权平均预测值 | 计算加权平均值用于排序配置或环境变量的物种得分
# https://www.rdocumentation.org/packages/vegan/versions/2.4-2/topics/wascores
...
还有另一种方法可进行 *双投影* (基于环境变量与 PCOA 排序轴的相关关系). 
由 Euclidean 距离计算而来的 PCoA 产生了对应于在相同数据的 scaling-1 PCA 双序图中获得的特征向量. 

分析包 ape 中可使用 pcoa() 和 biplot.pcoa() 函数. 

```{r}
# https://www.rdocumentation.org/packages/ape/versions/5.3/topics/pcoa
# 使用 pcoa() 函数对物种向量进行主坐标分析和预测
(spe.h.pcoa <- pcoa(dist(spe.h)))
# par(mfrow = c(1, 2))
# 第一个双序图: Hellinger-transformed species data
# https://www.rdocumentation.org/packages/ape/versions/5.3/topics/pcoa
# https://github.com/cran/ape/blob/master/R/biplot.pcoa.R
biplot.pcoa(spe.h.pcoa, 
            spe.h, 
            dir.axis1 = -1) # 反转轴1直接生成结果与 PCA 结果相当 - 相互比较; 排序轴的符号可任意
abline(h = 0, lty = 3)
abline(v = 0, lty = 3)
text(-0.5, 
     0.45, 
...
- **为了将物种数据投影到 PCoA 图上, 使用具有与用于计算相异度矩阵变换相同的变换(如果有的话)的物种数据**
...
对非欧式距离进行排序, 首选 PCoA; 分析包 ade4 在 Jaccard & Sørensen 相异性度量中计算了 相异度的平方根, 完全的欧式距离排序; 分析包 vegan 中使用 bray 方法进行百分量差异相异度计算, 其相异性度量可能不是欧式距离. 
这种情况导致了一些 **负特征值**. 
分析包 ape 中 pcoa() 函数可对其进行校正. 
...
如果排序图中的对象之间的确切的差异不是研究重点, 而是想探究排序关系在 小的 & 制定数量的 轴中 对象之间的相互关系, 非度量多维尺度分析(Nonmetric Multidimensional Scaling, NMDS) 可能会是较好的解决方案. 
与 PCoA 一样, NMDS 可以从任何相异性矩中 生成对象的坐标(或者说是进行排序分析). 
*该方法可以处理缺失值, 只要有足够的度量来相对于其他几个对象定位每个对象.* 
**NMDS 不是 特征值分析方法**, 并且它不会最大化与排序的各个轴相关的可变性. 
结果, 曲线可以任意旋转或反转。

分析过程如下:

- 指定排序的轴(尺寸)的所需数量 m ;
- 构造 m 维中对象的初始配置, 以用作迭代调整过程的起点. 这是一个棘手的步骤, 因为最终结果可能取决于启动配置.  PCoA 排序可能是一个很好的起点. 否则, 尝试使用**随机**初始配置的**许多独立运行**; 
- **迭代过程**尝试将对象定位在所请求的维数中(对象的位置), 以便最小化应力函数(stress function, 从0到1缩放), 该度量函数测量还原空间配置中的不相似性从单调到单调的程度. 关联矩阵中的原始差异; (**???**)
- 不断调整位置, 直到压力值(stress value)不能降低, 或直到达到预定的低值(容忍缺乏配合)
- 大多数 NMDS 程序使用 PCA 作为最终解决方案, 以便于结果的解释

对于少量轴(例如, m = 2 或 3), 在相同数量的维度中, NMDS 比 PCoA 获得更少的 对象之间的 相异性关系的 变形表示. 
NMDS 的迭代运算对计算资源的要求比较苛刻, 通常获得结果也只是局部最小值, 并非真正的最小值. 
### 5.6.2 使用非度量多维尺度分析鱼类数据集

使用分析包 vegan 中的 metaMDS() 函数进行非度量多维尺度分析(Nonmetric Multidimensional Scaling, NMDS); 
使用 wascores() 函数也可以将物种信息添加到排序结果图中; 
解释性变量可以通过 envfit() 函数进行添加.
...
- **注意比较分析 NMDS & PCA & CA & PCoA 之间的异同**

如果相异性矩阵中含有缺失值, 使用 isoMDS() 函数进行 NMDS 分析. 
默认情况下, isoMDS() 函数会调用 cmdscale() 函数的 PCoA 分析进行初始配置. 
为避免获得局部最小值, 建议使用分析包 labdsv 中的 bestnmds() 进行分析(该函数是 isoMDS 的一个包装器, 通过随机初始值进行分析; 该函数保留了最小应力值的解) 

通过 Shepard diagram 对 NMDS 分析结果图进行比较 -- 排序图中对象之间的相异性 与 原始相异性; 排序结果的优劣程度通过线性或者非线性回归的 R方 进行评估. 
使用分析包 vegan 中的 stressplot() & goodness() 都可获得相应评估参数. 

- https://www.displayr.com/goodness-of-fit-in-mds-and-t-sne-with-shepard-diagrams/
- http://www.analytictech.com/borgatti/mds.htm
- https://rdrr.io/cran/vegan/man/goodness.metaMDS.html
- https://rdrr.io/github/vmikk/metagMisc/man/shepard_plot.html
...
与其他的排序方法一样, NMDS 排序图中可以添加聚类结果的信息(根据分组通过颜色进行区分). 
...
两种方法具有相类似的目标: 获得小数量轴(2~3)上的对象的有意义的排序结果. 
PCoA 通过对已变换数据的相异性矩阵进行奇异值分解(eigen-decomposition)来分析, 
NMDS 通过迭代优化算法进行分析. 
两种方法的孰优孰劣尚无明确的评判. 
两种方法在所有的情景下, 产生的效用是否相同, 最后获得的解决方案是否一致? 
如何比较分析两种方法的基本规则和应用? 

> **NMDS**

- 迭代算法可以根据计算的起始点找到不同的解决方案, 在大多数情况下, 计算的起始点是随机选择的配置; 
- 在NMDS计算过程中, 相异性会被扭曲(拉伸或挤压) -- 方法的公认属性; 排序方案中的距离并不真正对应起始的相异性; 
- NMDS 的第一轴不一定能最大化观测值的方差. 大多数(如果不是全部) NMDS 程序计算 NMDS 结果的 PCA 旋转, 以便第一轴最大化 NMDS 解决方案的方差
- 同的NMDS功能可以使用不同的标准来最小化压力. 解决方案可能根据一个(或R函数)选择优化的标准而不同; 
- R函数可能包含可能影响解决方案的其他几个参数. 
除了经验丰富的用户之外, 很难确定哪种选项组合最适合特定数据集; 
- 用户必须设置k, 排序解决方案的维数. 可以产生所有的排序轴,即min [p，n 1], 但是由于距离是扭曲的, 所以解决方案不精确. 建议设置k <（n'1）
- 压力统计量不表示在排序解决方案中表示的数据的方差的比例。
相反, 它表示原始相异性的变形量, 这是非常不同的。

应用于生态分析 -- 当有必要将 3~4 维度的数据挤出 PCA or PCoA 分析中的二维时, NMDS 可以很好地表示二维中各位点之间主要的相异性关系. 
在这种情况下, 最好是使用 PCA 或 PCoA 排序轴作为 NMDS 的输入, 以确保 NMDS 解决方案不会明显偏离度量排序. 

> **PCoA**

- 通过奇异值分解寻找最优的解决方案 - PCoA 具有独特性; 
- 在排序分析过程中, 相异性不会被扭曲; 
- 通常根据相异性矩阵获取第一排序轴(方差最大化的轴); 
- 产生给定相异性矩阵的所有排序轴, 排序结果准确; 
- 通过 PCoA 的全部矩阵可以重建出对象的距离. 由 PCoA 计算而来的对象之间的欧式距离的矩阵 与 PCoA 的相异性矩阵严格相等. 
- 可重复性强. 
- 可计算 pseudo-R方 统计值 -- 感兴趣轴的特征值之和 除以 所有的特征值之和. 

应用于生态分析 -- 1> PCoA 对降维的二维或三维空间中的对象进行排序分析, 2> PCoA 还可以在计算适当选择的不同度量后充当数据转换. 
全维度 PCoA 空间中的对象坐标表示转换后的数据. PCoA 可以作为很多分析过程的起点, 比如 db-RDA & k-means 划分聚类等. 
**PCoA 可以再很多分析中使用**
...
- 1 - 计算原始或已中心化数据矩阵的协方差矩阵 S .
- 2 - 计算 S 的特征向量和特征值
- 3 - 提取特征向量的矩阵 U 并计算用于 scaling-1 双序图的主要成分的矩阵 F. 此步骤涉及已中心化的数据
- 4 - 计算矩阵 U2 和 G 用于 scaling-2 双序图
- 5 - 输出结果
