...
- **'条形图'中的'绿色'部分表示'按照该方案分区将得到重要的指示物种'**
...
*这个值是建立在所有 k 个分类组上的, 分类组中的所有对象的平方(Euclidean)距离的和, 每个值都除以分类组中的对象数量. *
这和 Ward 最小方差聚合聚类分析使用的标准一样. 

#### 4.8.1.1 k-means with Random Starts - 随机化的 k-均值

如果考虑到预先分组的数量, 可以使用 stats 分析包中的 kmeans() 函数. 
该函数可使用不同的随机初始值, 自动地重复大量(次数由参数 nstart 决定)的分析, 经过多次(次数由 nstart 决定)重复分析之后找到最优的解决方案(最小化的 SSE 值, 也就是最小的'总平方和误差')

k-均值分区方法是一种线性分析方法, 并不适合分析含有大量零值的原始物种丰度数据. 

(**第一种解决方案**)一种可能性是使用'非欧式相异矩阵(non-Euclidean dissimilarity matrices)', 比如百分比差异( Bray-Curtis ); 
这一类的矩阵应该进行平方化数值转换, 为能够获得对象在'欧式空间(Euclidean space)'中的所有表示, 需要将数值转换之后的数据进行主坐标分析(principal coordinate analysis, PCoA). 
将获得的 PCOA 轴 进行 k-均值分区处理. 

(**第二种解决方案**)另一种解决方案需要对物种数据进行预转换(对物种数据进行弦转换或者标准化,  chord-transformed 或 'normalized'). 
当与 k-均值中隐含的欧式距离相结合时, 在后续的分析过程中需要保留位点之间的弦距离. 

为了与上述 ward 最小方差聚类结果进行比较, 下文将 k 设置为 4 进行 k-均值聚类分析. 

```{r}
# K均值分解的物种数据预转换
# 由于 'nstart' 起始于不同的随机配置, 因此每次运行的结果可能会有所不一样
spe.kmeans <- kmeans(spe.norm, # 数据标准化转换
                     centers = 4, # 分成 4 个簇类 
                     nstart = 100) # 如果 centers 是整数,应该选取多少个随机数据集
head(spe.kmeans) 
# 通过列联表进行比较分析 | 比较聚类结果是否相似
# 使用交叉分类因子来构建每个因子水平组合的计数的列联表
...
(*上文中的'第一种解决方案'*) 
对于不能从原始数据转换后进行欧式距离(Euclidean distance)计算的相异指数(如 百分比差异相异矩阵, Bray-Curtis), 需要先将原始数据相异度矩阵进行主坐标分析(principal coordinate analysis, PCoA)以获得 n 行的数据表格, 然对获得的数据表格进行 k-均值聚类分析. 
对于百分比差异的相异矩阵(Bray-Curtis 相异矩阵), 首先需要对其平方根化然后在进行 PCoA 分析以获得完全的欧式解决方案, 或者使用提供负特征值校对的 PCoA 函数进行分析. 

划分区处理可产生一个具有预定义的分组数量的简单分区. 
可以通过尝试不同的分组数量进行多次重复分析. 

只有首先确定'最好聚类方案的选择标准', 才能选出'多少分组(聚类簇)数量才是最好'. (*磨刀不误砍柴工*) 

可以使用cclust 分析包中的 clustIndex() 函数获得多种聚类簇选择的标准. 
1) Calinski 指数: 比较组间和组内分区的平方和 (由于聚类簇内所含对象数量的不一致性或者说是差异较大, 导致 Calinski 指数可能会比较低); 
2) SSI (Simple Structure Index), 简单结构指数的最大值也是衡量最小二乘意义下的最优分区结果的选择标准. 

使用 vegan 分析包中的 cascadeKM() 函数, 可以避免多次简单调用 kmeans() 的繁琐操作, 同时该函数也是 kmeans() 函数功能的升级应用(可以实现一定聚类簇数量范围之内的重复调用 kmeams ). 

- **不同的颜色表示不同的聚类簇, 相同的颜色表示属于同一个聚类簇.** 

- *'总平方和误差'的最小值是寻找对象最优分组的准则 (在给定 k 中进行选择), 而 calinski 和 ssiare 值是寻找 k 值(到底选择多少的分组结果)最优解的判断依据 (在一个范围内确定最佳 k 值, 最小值决定最佳 k 值). *

- 虽然 k 设置为不同的数值, 但在计算比较的时候独立进行分析

- 由于连续分区的组并不一定是嵌套的, 所以自上到下的检查并不等同于检查树形图. 

- 一旦确定了位点的聚类簇, 接下来就是物种数量的信息分析. 最简单的方法就是根据保留的类型特征来定义位点的子分组, 并统计基本信息. 

```{r}
# k-均值划分 2 ~ 10 簇类
# cascadeKM 是 vegan 分析包中的 函数
# 使用一系列的 k 值进行k-均值划分; 并提供一定的数值统计
...
plot(spe.KM.cascade, 
     sortg = T) # 是否对簇类进行排序 | 按照紧密程度重新进行排序
plot(spe.KM.cascade, 
     sortg = F) # 是否对簇类进行排序 | 不按照紧密程度重新进行排序
...
从另一种角度来看, k-均值聚类分析可被用来优化层次聚类结果. 
事实上, 聚合算法本质上组织了组内对象向后续聚合分析新产生组的转移, 即便后续聚合分析的结果更加适合. 
为了克服这个潜在的问题, 我们可以向k-均值算法提供从要优化的聚类中得到的先验信息, 
要么是以用该方法获得的 k 个组中 p 个变量均值的 k*p 矩阵的形式, 要么是以 k 个对象的列表的形式; 
每组生成一个'典型'对象. 

在后面的一节中, 这种'典型'对象被称为'中心点划分', 视为用于构建群体的种子. 使用该方法验证鱼类物种数据经过 ward 最小方差聚类分析获得的 4 个分组是否经过了 k-均值聚类分析的调整. 
...
**"在数据集的观测值中筛选 k 个具有代表性的对象或中心点. 这些观测值应该代表数据集的结构. 在确定一系列 k 个中心点之后, 通过将每个观察值分配(制定)给最近的中心点来构建聚类簇. 这一系列操作的目的在于找到 k 个代表性的对象, 并且这些代表性对象可以最小化观察值与其最近代表性对象的相异之和"** -- pam() 函数描述

与之不同的是, k-均值聚类分析中最小化的是组内欧式距离的平方和, 因此, k-均值聚类是纯铜的最小二乘方法, 然而, **PAM 并不是最小二乘方法**.

cluster 分析包中的 pam() 函数既可以输入原始数据, 也可以输入相异矩阵; 然而, kmeans() 函数只能输入变换之后的数据矩阵; 同时, pan() 函数可允许通过轮廓宽度准则来选择最优的分组数量. 
...
- 由于 PAM (围绕中心点划分)是最小化相异系数的和, 而不是欧式距离的平方和, 因此 pam() 方法更加'稳健'. 

- 在给定分组数量之后, PAM 方法更倾向于通过一系列初始中心点收敛至相同的解决方案. 而然, 获得的方案并不能保证是最适合研究目的的解决方案. 

- 选择何种方案, 取决于哪种方法的结果可以提供更多的相关信息, 或者能够更好地被环境变量解释. 

---

## 4.9 Comparison with Environmental Data - 比较环境数据

上文中已经使用物种丰度数据为例简单介绍了多种聚类方法. 
多种方法也可以应用于其他类型的数据, 尤其是环境数据表. 
使用过程中一定要格外注意每个变量正确编码的选择和数据转换以及关联测度. 

### 4.9.1 Comparing a Typology with External Data (ANOVA Approach) - 比较类型学与外部数据（方差分析）

通过前文中的描述, 可得知内部准则(比如仅仅依赖物种数量的轮廓宽度和其他聚类质量指标)并不总是能够选择位点的分区结果.
**拓扑学的最终选择应该基于分类组的环境可解释性. **
这可以看做是位点类型学的外部验证. 

可以通过判别分析(discriminant analysis)将聚类结果(通常被视为响应数据)与外部独立性数据结合, 进而进行分析. 
从另一个角度看, 从社区组成型数据中获得的聚类可以被看做是方差分析意义上的一个因素或者分类标准. 

本章将对几个环境变量分别进行方差分析假设(残差正态性和方差齐性, normality of residuals and homogeneity of variances)的快速评估, 分析过程中或使用带参数的方差分析, 或者使用非参数的  Kruskal-Wallis 检验. 

通过使用箱式图来展示优化的 ward 聚类簇. 

值得注意的是, 基于群落物种组成数据的聚类结果在方差分析中扮演一种解释变量的角色(因子角色), 但在生态学角度看来, 真正寻找的是成组位点中的环境解释. 

```{r}
# 展示鱼类数据聚类结果(基于最优分区)与环境变量之间的相关关系

# 针对特定数据框进行分析 | with() 避免重复使用数据对象
with(env, {
  # 箱式图 - 环境因子
  par(mfrow = c(2, 2))
  boxplot(
...
with(env, {
  ...
  # 变量 sqrt(ele) 具有异构差异, 不适合进行有参方差分析(parametric ANOVA)
...
后验试验并标记分类簇的显著性. 
函数 boxplert() 可以展示方差分析(ANOVA)和 LSD 检验的多重比较, 
而函数 boxplerk() 用以表示 Kruskal-Wallis 检验和相对应的事后验证比较(**两者都是用 Holm 校正**)
...
- **在此分析的基础上, 如何确定四种鱼类群落类型的生态类型??**

当然, 也可以进行相反的分析过程. 
先聚类环境变量(获取一系列的生境特征), 然后通过指示物种分析的方式检测物种是否显著响应这些栖息地类型(4.11). 
在这种方法中, 物种逐一针对栖息地类型进行测试. 
如果对多个物种进行独立测试, 请考虑多次测试的问题.
作为替代方案，将在第6章中提出基于排序的多变量方法，以直接模拟和测试物种 - 栖息地关系

### 4.9.2 Comparing Two Typologies (Contingency Table Approach) - 比较两种类型（列联表方法）

如果仅想简单比较由物种数据产生的类型学和由环境变量独立获得的类型学, 可以使用列联表的形式进行类型学特征的比较和使用 Fisher extract 测试. 
...
目前有很多方法可以解决数据集中物种的关联性问题. 

### 4.10.1 Simple Statistics on Group Contents - 组内数据的简单统计

先前的章节中介绍了一种定义粗放型集合的方法: 
一是计算由聚类方法获得的类型学的基本统计量(例如实例平均值), 
二是寻找在位点的每个聚类簇中广泛存在,高丰度或者独特的物种.
...
Kendall’s W 一致性系数与置换检验相结合, 用以确定物种在丰度数据集中的组合情况(这种方法不可应用于二元'有-无'数据). 
**首先需要对所有物种进行独立性检验. 如果所有物种之间相互独立(拒绝零假设), 可以使用置换检验的方式进行组内物种的相关性分析并检验每个物种的统计贡献. **
在该方法中, 物种关联性的搜索既不涉及已知位点的类型特征, 也不由其他数据(比如示例环境数据)进行计算. 
该方法旨在找到最包容的组合, 最少聚类簇(簇中包含最大数量的显著正相关的物种). 

**当判断的数量[=物种数量]很小时(Kendall’s 一致性系数检验应用的实际场景多是如此), 经典的卡方检验(classical χ2 test)过于保守, 而置换检验可以校正 I 型错误; 置换检验的效力因此也就更高. **

简单示例: 使用 k-均值聚类分析鱼类物种数据集, 执行全局检验(kendall.global())验证是否所有物种的分组是全局显著关联的. 
如果所有物种的分组全局显著关联, 则对每个分组的物种进行后验概率检验(post hoc tests, kendall.post())进而确保组内的所有物种是否具有一致性(concordant).
...
# 注意校正之后的置换 p-值, 如果小于等于 0.05, 则认为所有的分组具有全局的显著性, 也就认为分组中所包含的物种是一致性的. 
# 但是, 这并不意味着在全局显著性的分组中的所有物种都是一致性的, 仅仅只有一小部分是一致性的. 
# 如果, 一些分组的校正 p-value 没有显著性, 也就是说这些分组中包含非一致性的物种, 同时应该更加细致地划分至更小的分组. 

# 后验试验 - 计算稍慢 | 鉴定每个分组内显著性一致的物种
# https://www.rdocumentation.org/packages/vegan/versions/2.4-2/topics/kendall.global
(spe.kendall.post2 <-
...
# 查看每个独立物种的平均 Spearman 相关系数 
# 如果每个物种与分组内的其他物种具有正平均相关性, 那么该分组中就含有一致性物种. 
# 如果物种与分组内的其他成员具有负平均相关性, 则将该物种从该分组中剔除. 
# 尝试更加细致的划分, 以期同组内的物种成员具有正平均相关性. 这种情况下很可能造成单一物种构成单一分组. 
...
在生态关系中, 生态学理论克预测出嵌套结构. 
在群落中, 物种的亚组要么紧密关联要么毫不相干. 
可以通过 Kendall W 检验揭示大量物种关联中的少量物种分组来探索这些途径. 

这里所定义的物种的分组可以通过多种不同的方式进行生态学解释. 
比如, 绘制物种沿河流的丰度图, 并统计位点的物种组合, 可以帮助评估物种的生态作用. 
另一种解释方法, 根据一组解释性环境变量计算显著关联物种的冗余分析( redundancy analysis, RDA ). 

### 4.10.3 Species Assemblages in Presence-Absence Data - 二元数据类型中的物种组合

使用新方法计算 R 模式下的 Jaccard’s S7 系数的组分 a, 并通过 Raup & Crick 系数置换检验组分 a 的概率. 
p-value 可以扮演一种相异性的角色: 对于高度贡献物种, 值很小. 

*过高的置换次数, 虽然可提高更加细致的评估, 但需要消耗计算时间*
...
# 与 adj.sigth 对应的未修正的 p-value 是 0.00017. 
# 因此, 显著性值应该更小
# 在概率矩阵中将大值使用 1 代替
...
# 统计 小于等于 sigth 的 未校正 p-value 数量
...
在群落生态研究中, 物种共现网络分析变得越来越流行, 尤其是在研究物种间或者群落中的生态相互关系领域更为流行. 
分析物种间关联性的基本原则主要是在生态'宏环境'或者多营养物种组合中共同存在. 
基于共现网络的拓扑结构, 将物种的'社会属性组'定义为'模块'. 
网络结构通常有两方面的主要特征, 
一是模块性, 物种高度相关联, 物种间没有重叠; 
二是嵌套性, 少量物种组合构成大量物种组合的子集 (俄罗斯套娃). 

物种的作用由其自身模块中的其他物种相比的位置来定义的 
("标准化的模块内度", 即节点与同一模块中其他节点的链接数量, 模块中每个节点连接数量的标准偏差和均值进行标准化, 以及该物种如何与其他模块中的物种相连接(模块间"连接"). 

基本上, 通过邻接矩阵来构建网络(这种邻接矩阵可以是二元'有-无'矩阵类型--物种显著性共现或者不共现; 也可以是数值型的矩阵类型--物种间的连接具有加权信息). 

物种间的正相关,中性相关和负相关的多种指标都可以用来用以计算包括 Spearman 相关系数,Jaccard 相似系数或者前文中的 p-value 在的邻接矩阵. 
这些指标可以从物种的'有-无'二元矩阵或者丰度矩阵中计算获得. 
通常应用阈值来将非零链接限制为最重要的正关联以构建**无向网络(undirected networks)**. 
经常共现的物种在网络图中彼此邻近. 

本章使用 picante 分析包计算共现距离, 使用 igraph 分析包进行网络处理. 
...
在应用生态学中, 搜寻多个位点处的指示性物种或者特征性物种具有格外突出的重要性, 
比如可以快速评估或者预测给定位点的保守和维持性物种, 
评估给定区域中活的群落的类型, 
亦或是检测一些环境参量的变化. 
在基础生态学中, 可以查看一系列中位点中物种的生态偏好, 就可以增加对相应物种的了解, 也可以增加对采样处栖息地生境的了解. 

研究中使用的方法具有互补性, 可以使用不同的方法尝试去解释. 
De Cáceres 和 Legendre (2009) 提出了两种互补性质的方法以及他们相关的指标(**指示值**和**相关性指标**)

**指示者至指标** 是基于**特异性**和**保真度**两个概念提出的(特异性 -- 某物种在特定类组或者位点中最高而在其他类群中无; 保真度 -- 某类群在目标组中的所有位点都有存在且高). 结合高特异性值和高保真度值可以获得高指示者值. 

**相关性指标** 是基于 Pearson's 相关性系数计算而来(不同位点物种的有无). 

De Cáceres 和 Legendre (2009) 一共描述了两组中共计 12 个指标. 他们认为: 
指示者值在生态监测或者群落类型的确定中更有利于作为生物指示者进行评估或预测, 
而相关性指标应该用于确定系列位点中给定物种的生态学偏好. 

### 4.11.2 IndVal: Species Indicator Values - 物种指示值

**IndVal** 是目标类组中物种特异值和物种保真度的乘积. 
**特异性** 通过目标组内物种的平均丰度与其在各个组内的平均丰度相比较而定义; 
**保真度** 是物种存在的目标组所在位点的比例. 

> **indval** 是寻找既必要又充分物种的一种方法(如果发现那个物种, 应该属于哪个类型; 如果属于那个类型, 应该找到那个物种) 

可以应用多种方法定义位点组. 

循环法是一种使用基于物种数据的聚类分析结果的方法. 
指示者物种是这些组中最为突出的成员. 
由于分类结果和指示者物种并**不是直接从独立性数据**中得来的, 因此并不能仅仅依靠统计检验来确定指示性物种. 

同时还有一种在概念和统计上更有效的方法, 基于**独立型数据(比如环境变量)**对位点进行聚类. 
可将通过该方法所获得的指示者物种视为真正意义上的指示者物种(类组中与其生态条件密切相关的物种). 

可通过置换试验来评估指示者值的统计学显著性. 

分析包 labdsv 中的 indval() 可以用于计算指示者值. 
...
- **注意:** 输出表中的聚类编号不一定有意义. k-均值聚类分析以数字的形式差生任意的组标签. 列标签进行了重新排序. 

- 聚类分组的结果不一定按照河流的顺序, 就是为什么要对对象进行有序的组编号. 

- 表中包含的信息:

1. relfrq = 每个组中物种的相对频率 = (物种存在位点的数量)/(组中位点的数量)

2. relabu = 所有组中物种的相对丰度 = (组中的总丰度)/(巨大总丰度) total abundance in
group/grand total abundance

3. indval = 每个物种的指示者值

4. maxcls = 具有最高指示者值物种所在的聚类簇编号

5. indcls = 物种最高的指示者值

6. pval = 置换检验的 p-value
...
- 鉴定的指示者物种可能与 4.10 章节中使用的物种组合方法的结果不一致

- 指示者物种与预定义组有关联, 而物种组合是在没有预先分类和参考环境条件下分析的. 

*链接 4.12 回归树分析 MRT 相关的分析*

分析包 **indicspecies** 可用于计算各种指标物种指数, 该分析包有两个有趣的功能: 
1) 汇总两个或者多个类型学的分组以寻找可能是汇集组的指示者物种 -- multipatt(); 
2) 通过自展分析计算指示者值--strassoc(). 
...
1. comb = 位点与所有可能组合(取决于分析中要求的顺序)的矩阵; 

2. str = 相关性的强度;

3. A = IndVal 分析前提下表示 "特异性"; 其他情况无意义;

4. B = IndVal 分析前提下表示 "保真度"; 其他情况无意义;

5. sign = 最适合模型的结果, 统计结果和置换试验结果最优的组或者组合组; 不提供多重检验校正. 
...
# 汇总统计信息
# 第二次汇总统计的结果与第一次的汇总统计结果有所不同
# 具体体现: 具有高 IndVal 值的一些物种形成组合组, 代替了原先的单个组
...
置信区间的下限为 0, 稍微大一点的都视为不重要. 
区间的范围可能会有重合, 但是不能认为最终的值就不相同; 真实的指示者值可能在交集之中, 尚无办法确定值的大小(重叠的范围无法判断大小). 

### 4.11.3 Correlation-Type Indices - 相关类型指标

相关性指标用以帮助确定系列组中物种的生态学偏好. 
以生态学偏好鉴定为目的, 使用相关性指标的效果要好于指示者指标, 得益于相关性指标允许检测阴性偏好. 

要注意位点数据中的物种缺失相关的问题. 在不同的位点, 物种缺失的原因可能会不同, 因此不能草率地将缺失物种信息用于生物学意义的解释. 

通常将植物学中代表明确生态条件的的特定组中具有高度偏好性的物种称为"诊断性"物种, 有助于野外调查中植被类型的识别. 

对于二元数据类型('有-无')的相关性类型指标通常称为 Pearson 相关性系数. 
计算包含在两个二元(1 或者 0)向量间的相关性. 

使用定量(丰度)数据时, **Pearson's phi** 相关系数称为'点双相关系数', 
丰度向量代替'有-无'向量. 
...
# 统计每列的基本信息
# 结果与 IndVal 分析的相似 | 使用两种方法揭示了某些物种和选定组之间的强相关性
# 统计检验中显示了最高的"正 Pearson's phi 相关系数值"
summary(iva.phi) 
...
多元回归树是一元回归树的扩展版本, 在定量或者类别解释性变量控制下可对定量响应变量进行递归分割. 
有时也成这种方法为约束性或者监督性**聚类**. 
分析结果以**树**的形式呈现, 其中, 叶子(末端位点组)是依据组内平方和最小的原先选取的位点组合, 但是每个连续性分区通过一个阈值或者可解释性变量来决定. 
在诸多候选的方案中, 依据组的组成和叶端数量, 保留预测效能最好的方案. 
**多元回归树分析的选择依据是--预测效能; 第六章中介绍的诸多约束性排序方法则是依据--可解释性效能进行方案的选择.**
介于使用的是预测效能, 因此在环境管理中经常使用多元回归树模型(MRT). 

MRT是一种健壮且多面型的分析方法, 可以适应多种场景, 
某些数值缺失, 解释性变量与响应变量呈现非线性关系, 或者解释性变量之间存在交互项(? 多重共线?)

### 4.12.2 计算原理

MRT (多元回归树方法)包括两部分的分析过程:
**1)** 对数据进行约束性分区; 
**2)** 交叉验证结果. 

#### 4.12.2.1 Constrained Partitioning of the Data - 约束性的数据分区

-1. 对于每个解释性变量, 将位点间所有可能的分区换成两组. **1)** 定量变量: 依据变量的有序值对位点进行排序, 在第一个对象,第二个对象之后反复拆分; **2)** 分类变量: 将对象分成两个组, 并筛选所有可能的组合. 
在所有的情况下, 计算响应数据的组内平方距离和与组均值(组内SS)的和. 
保留和最小的分组方案, 以及解释变量的标识与值, 产生分区的分类变量的级别. 

-2. 对上述保留的方案中的两个亚组内重复相同的操作, 在每个分组中保留最优的分区方案以及相应的解释变量和阈值; 

-3. 在所有分区内继续上述操作, 直至所有对象形成各自的分组或者直到获得每组对象的预选最小数值. 选择适合研究的树的大小. 使用交叉验证识别最佳的预测树. 

-4. 除了叶子的数量和组成外, 相对误差(relative error, RE)也是树的一个重要特征, 所有叶子的组内平方和(with-in group SS)的和 除以 原始数据的平方和(SS). (相对误差就是不能通过回归树解释的方差的比例). 在没有进行交叉验证( cross-validated)时, 连续性的分区分析将保留相对误差(RE)最小的解决方案, 等同于保留了 R方(决定系数) 最大化的解决方案. **然而,** 这将是解释性的方法而非预测性的方法. **相对误差(RE)方法对新数据预测准确度的估计过于乐观, 可以从交叉验证相对误差(CVRE)的分析中获取更优的预测精度. **

#### 4.12.2.2 Cross-Validation of the Partitions and Pruning of the Tree - 分区块的交叉验证与树形的修剪

应该在哪一层剪去一颗树, 从而保留最敏感的分区结果. 
为了以预测为导向的方式回答这个问题, 使用对象的子集(用作训练集)构造树, 然后将其余的对象(测试集或者验证集)分配给所构造成树的对象来验证结果. 

预测误差的度量方式是交叉验证相对误差(CVRE), 也可以将其定义为一种比值(即: 通过树无法解释的方差**除以**响应变量总方差). 因此, 在每个分区处理之后, 分子都会有所变动. CVRE 趋于 0 表示越是完美的预测结果, 趋于 1 则表示预测效果越差. 

#### 4.12.2.3 MRT Procedure - 多元回归树方法的分析过程

MRT 分析过程主要包括两部分, 下面的内容将其整合在一起进行介绍:

- 1. 随机分割成 k 组; 默认使用 10; 

- 2. 选择一组通过约束性分区构建树, 基于组内最小平方和(SS)进行决策; 

- 3. 重复上述步骤 k-1 次, 依次省略每个测试; 

- 4. 在 k 个解决方案中, 对于这些解决方案中的每个可能分区大小(组数), 重新分配测试集. 计算 k 个解决方案的所有分区大小; 包括分区水平和所有 k 个解决方案水平的计算. 

- 5. 修剪树: 保留 CVRE 最小化分区的大小. 另一种替代方案是: 最小 CVRE 值 **加** CVRE 值的一个标准误差 **作为新的 CVRE 值** -- "1 SE 准则". 

- 6. 为获得这个过程的误差估计, 将对象对随机化分配给 k 个组, 并且重复多次(100 或 500)

- 7. 最终树中保留了所有置换分析中最小的 CVRE 值, 或者最符合"1 SE 准则"的一棵树. 

- 8. 在 mvpart() 中, 当参数 minauto = TRUE时, 一组中最小可能的对象数等于ceiling(log2(n)). 设置 minauto = FALSE 允许分区继续进行, 直到数据集完全拆分为单独的观察结果. 

在多元回归树分析(MRT)中, 平方和(SS)是使用 Euclidean 空间距离计算而得. 
为能够解释数据的特殊特征, 需要对数据进行预处理(章节 2.2.4 中有相关的内容介绍). 
对于使用不同物理单元进行环境描述的响应数据, 在进行 MRT 分析之前需要对其进行标准化. 
...
对结果的总结信息提供了每个节点的贡献度(用以解释方差 -- 复杂性). 
这些数值的总和称之为 总 R 方. 
此外, 还可确定每个节点处的"判别式"物种 -- 对解释方差的贡献最大. 
同时, 在总结的信息中也提供了每个节点的两个分支的平均丰度信息(已进行数据转换), 因此可以观察分支的"判别式"物种.

### 4.12.4 Combining MRT and IndVal - 结合多元回归树方法与指示值方法

这比识别“判别式”物种的可视化效果更好好, 可以使用Holm校正来测试指标值的显着性并校正多次测试的 p 值. 
...
多元回归树分析为单元聚类提供了一个解决方案, 即一种无约束的聚类形式(其中, 树的每个筛分选择一个单一的响应变量). 
使用物种数据作为响应和解释性变量来构建回归树. 
这种方法可将同质化组成的位点和通过递增化的指示者物种的存在(或者相对丰度)来解释的位点划分成不同的组. 

这种方法可以应用于'有-无'数据类型或者预转换的丰度响应数据; 解释性数据可能是二元('有-无')类型或者定量类型(未转换或已转换), 相同的数据矩阵经过多种不同的数据变换之后也可以应用. 
...
在数据以空间(横断面)或时间顺序呈现的情况下查找组时可以考虑邻接信息, 并且在连续性时可以考虑连续性信息以及系列. 

连续性聚类可以通过 MRT 的方式进行计算. 

本小节将应用为地层研究开发的具有邻接约束的聚类方法, 并称为 CONISS. 使用分析包 rioja 中的 chclust() 进行分析.
...
在本章的开头, 将通过聚类方法或者的聚类簇定义为非重叠的实体. 
这种定义是关注不连续性的结果(**如果关注连续性会有什么样不同的结果?**). 
然而, 还有另外一种聚类方法, 有时聚类簇限制(界限)没有想象中的那么清晰. 
在这种情况下, 同一个对象可能在不同程度上属于两个或者多个组的成员. 
因此, 本小节提出了一种不同的层次和非层次方法, 即模糊聚类方法. 
以下内容仅作相关简短的介绍. 

### 4.15.1 Fuzzy c-means Using Package cluster’s Function fanny() - 使用 cluster 分析包中的 fanny() 函数计算**模糊 c-均值**

与传统一个对象仅属于一个聚类簇不同, c-means 聚类将一系列成员值与所有对象相关联, 以衡量其在各种集群中的成员关系的强度. 
一个对象可以与某一聚类簇有强相关性, 同时也可以与另一个分类簇具有相对较弱的相关性.

两种分析分析方式: cluster 中的 fanny() 和 e1071 中的 cmeans(). fanny() 既可以接受'位点-物种'类型数据, 也可以接受'相异性矩阵'.
...
在不同的模型下, 对群落数据应用非层次或层次模糊聚类分析的方法. 
噪音聚类 -- 它允许考虑异常值，即未分类的对象
...
虽然“硬”聚类在生态学中可能看起来有点不切实际, 但当需要一个需要明确分配站点的类型学或决策工具时, 它的应用会有很大的帮助. 

如果模糊聚类的目的是描述站点之间的相似关系, 则模糊聚类在大多数生态环境中是一种更细微的,更加真实的方法. 
...
本章节不可能穷尽现有所有的聚类分析方法. 
不同的生态学研究方向都有本领域的一些**特点特色和限制因素**, 因此在聚类分析方法的选择上应该**因领域而异, 因研究方向而异, 因研究目的而异**.
